...
const linkedinAccount = {
  email: 'susanfancher@outlook.com',
  password: '5MNCrWKcZVjL3',
  emailPassword: '2ukU4sTLV',
  recoveryEmail: 'susanfancher@mailnesia.com'
}
...
const main = async () => {
  ...
  const verifCode = await mailnesiaScraper(
    browser,
    getNameFromEmail(linkedinAccount.recoveryEmail)
  )

  if (!verifCode) {
    console.log('failed to find verif code from mailnesia')
    await browser.close()
    return
  }
